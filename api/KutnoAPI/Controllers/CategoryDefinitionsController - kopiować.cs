...
namespace KutnoAPI.Controllers
{
	...
	public class FinancialDataController : ControllerBase
	{
		...
		private string SCHOOLS_BASE_QUERY = "SELECT * FROM pgj.financial_summary";

        private const int RECORD_DEFAULT_LIMIT = 100;

        private readonly ILogger<FinancialDataController> _logger;

		public FinancialDataController(ILogger<FinancialDataController> logger, IConfiguration configuration)
		{
			_connectionString = configuration["DbString"];
			_logger = logger;
		}

        [HttpPost(Name = "GetFinancialSummaries")]
        public async Task<ActionResult<IEnumerable<FinancialSummary>>> getValues(SearchRequest searchRequest)
        {
            ...
            FinancialSummary temp = new FinancialSummary()
            {
            };
