...
namespace KutnoAPI.Controllers
{
	[ApiController]
	[Route("[controller]")]
	public class CategoryValuesController : ControllerBase
	{
		private string _connectionString = string.Empty;
		private string SCHOOLS_BASE_QUERY = "SELECT * FROM pgj.category_values";
        private string COMPUTED_BASE_QUERY = "SELECT * FROM pgj.computed_category_values;";

        private const int RECORD_DEFAULT_LIMIT = 100;

        private readonly ILogger<CategoryValuesController> _logger;

		public CategoryValuesController(ILogger<CategoryValuesController> logger, IConfiguration configuration)
		{
			_connectionString = configuration["DbString"];
			_logger = logger;
		}

        [HttpPost(Name = "GetCategoryValues")]
        public async Task<ActionResult<IEnumerable<CategoryValues>>> getValues(SearchRequest searchRequest)
        {
            ...
            using (var connection = new NpgsqlConnection(_connectionString))
            {
                ...
                var result = await connection.QueryAsync<CategoryValues>(query, parameters);
                return Ok(result);
            }
        }
        [HttpPost("computed")]
        public async Task<ActionResult<IEnumerable<CategoryValues>>> getComputedValues(SearchRequest searchRequest)
        {
            var query = string.Empty;
            var paramDict = new Dictionary<string, object>();
            paramDict.Add("@limit", searchRequest.Limit > 0 ? searchRequest.Limit : RECORD_DEFAULT_LIMIT);
            ...
            if (searchRequest.Conditions == null || searchRequest.Conditions.Count == 0)
            {
                query = String.Format(COMPUTED_BASE_QUERY, "", orderStatement);
            }
            else
            {
                var invalidColumns = searchRequest.Conditions.Where(c => !propertiesNames.Contains(c.Column)).Select(c => c.Column).ToList();
                ...
                var new_dict = ConditionParser.ParsePostgres(out where_string, searchRequest.Conditions, temp);
                paramDict = paramDict.Concat(new_dict).ToDictionary(kvp => kvp.Key, kvp => kvp.Value);
                query = String.Format(COMPUTED_BASE_QUERY, where_string, orderStatement);
            }
            using (var connection = new NpgsqlConnection(_connectionString))
            {
                var parameters = new DynamicParameters(paramDict);
