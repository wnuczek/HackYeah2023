...
PACKAGE_DEFINITION = ['DOCHODY.XML', 'EGZAMINY.XLSX', 'ETATY.XML', 'SZKOŁY.XML', 'WYDATKI.XML']
                ...
                for directory in dirs:
                    if directory.isdigit():
                        ...
                        with os.scandir(os.path.join(ROOT_DIR, directory)) as content_dir:
                            content = [i.name.upper() for i in content_dir if i.is_file()]
...
def PullFiles(year, path):
    ...
    x = {"year": year, "schoolsworksheet": base64.b64encode(blob_data4).decode(), "jobsworksheet":  base64.b64encode(blob_data3).decode()}
    # print(x)
    ...
    x = {"year": year, "incomesummary": base64.b64encode(blob_data1).decode(), "costssummary":  base64.b64encode(blob_data5).decode()}
    payload = json.dumps(x)
